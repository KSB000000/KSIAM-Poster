...
class Blip2T5InstructQformerLLMLoRA(Blip2Base):
    ...
    def forward(self, samples):
        ...
        # 보기(Choices) 임베딩 처리
        for choice_key, choice_img in samples["choices"].items():
            choice_img_embeds = self.ln_vision(self.visual_encoder(choice_img))

            query_output = self.Qformer.bert(
                query_embeds=query_tokens,
                encoder_hidden_states=choice_img_embeds,
                encoder_attention_mask=image_atts,
                return_dict=True,
            )
            choice_img_embeds = query_output.last_hidden_state[:, :32, :]
            choice_img_embeds = self.t5_proj(choice_img_embeds)
            choices_embeds.append(choice_img_embeds)
            choices_atts.append(torch.ones(choice_img_embeds.size()[:-1], dtype=torch.long).to(image.device))

        choices_embeds = torch.stack(choices_embeds, dim=1)  # (batch, num_choices, 32, 2048)
        choices_embeds = choices_embeds.view(choices_embeds.shape[0], choices_embeds.shape[1], 32, -1)  # (batch, num_choices, 32, 2048)
        choices_embeds = choices_embeds.squeeze(2)  # (batch, num_choices, 2048)
        choices_embeds = choices_embeds[:, :, 0, :]  # [batch, num_choices=4, embed_dim=2048]

        choices_atts = torch.stack(choices_atts, dim=1)  # (batch, num_choices, 32)
        inputs_t5 = inputs_t5 / torch.norm(inputs_t5, p=2, dim=-1, keepdim=True)
        choices_embeds = choices_embeds / torch.norm(choices_embeds, p=2, dim=-1, keepdim=True)

        similarity_scores = torch.bmm(inputs_t5, choices_embeds.permute(0, 2, 1))  # [batch, num_choices]
        similarity_scores = similarity_scores.mean(dim=1)  # (batch, num_choices) → num_tokens 축을 평균냄

        # ✅ targets 준비
        # `samples["text_output"]`에서 실제 정답을 찾고, 해당 인덱스를 targets로 사용
        num_choices = similarity_scores.shape[1]  # 예: num_choices = 4

        # samples["text_output"]이 1-based 인덱스라고 가정
        true_indices = torch.tensor([int(x) for x in samples["text_output"]], dtype=torch.long, device=similarity_scores.device)

        # 1-based 인덱스를 0-based 인덱스로 변환
        true_indices = true_indices - 1  # 1-based -> 0-based

        # 정답 인덱스를 유효한 범위 내로 조정 (0~3)
        true_indices = torch.clamp(true_indices, 0, num_choices - 1)  # 0 ~ num_choices-1

        # CrossEntropy Loss 계산
        loss = torch.nn.functional.cross_entropy(similarity_scores, true_indices)

        return {"loss": loss, "pred_probs": similarity_scores}
