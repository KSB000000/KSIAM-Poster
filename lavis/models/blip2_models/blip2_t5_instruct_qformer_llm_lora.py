...
class Blip2T5InstructQformerLLMLoRA(Blip2Base):
    ...
    def __init__(
        ...
        self.few_shot_prob_train = few_shot_prob_train
        self.few_shot_prob_eval = few_shot_prob_eval
    ...
    def forward(self, samples):
        ...
        if self.training:
            prob = self.few_shot_prob_train
        else:
            prob = self.few_shot_prob_eval

        fs_embeds, fs_atts = None, None
        if prob > 0 and "few_shot_samples" in samples.keys():
    ...
    def prepare_few_shot_embeds(self, samples):
        ...
        #     weights=[1 - prob] + [prob / self.num_few_shot_examples] * self.num_few_shot_examples
        # )[0]
        this_n_fs = min(len(samples), self.num_few_shot_examples)
        ...
        # # 확률 기반 개수 선택 유지하고 싶다면:
        
        # for fs_sample in samples[:this_n_fs]:
        #     images.extend(fs_sample["image"])
        #     text_input.extend(fs_sample["text_input"])


        selected_samples = random.sample(samples, k=this_n_fs)
        for fs_sample in selected_samples:
            images.extend(fs_sample["image"])
            text_input.extend(fs_sample["text_input"])
    ...
    def generate(
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            if fs_embeds is not None:
                ...
                fs_embeds = fs_embeds.expand(B, -1, -1)
                fs_atts = fs_atts.expand(B, -1)
        ...
        **kwargs
        ...
        if prompt:
            if prompt.count("{}") == 2:
                ...
                elif 'options' in samples:
                    text_input = []
                    for i in range(len(samples["text_input"])):
                        this_choices = [f"({string.ascii_lowercase[j]}) {ch}" for j, ch in enumerate(samples["options"][i])]
        ...
        if isinstance(output_text, list):
            ...
            for s in output_text:
                last_index = s.rfind("Final Answer:")
                if last_index != -1:
                    final_answer = s[last_index + len("Final Answer:"):].strip()
                else:
                    final_answer = s  
                processed_answers.append(final_answer)
        else:
            last_index = output_text.rfind("Final Answer:")
            if last_index != -1:
                processed_answers = output_text[last_index + len("Final Answer:"):].strip()
            else:
                processed_answers = output_text
    ...
    def from_config(cls, cfg):
        ...
        few_shot_prob_eval = cfg.get("few_shot_prob_eval", 0.0)
        few_shot_prob_train = cfg.get("few_shot_prob_train", 0.0)
        ...
        with lora(r, alpha, dropout, enabled=self_attention_qv_lora, qkv=[qformer_crossattention_lora_q, qformer_crossattention_lora_k, qformer_crossattention_lora_v]), custom_lora(r, alpha, dropout, enabled=(self_attention_output_lora or qformer_crossattention_lora_o), type="BertSelfOutput", sc=[self_attention_output_lora, qformer_crossattention_lora_o]), custom_lora(r, alpha, dropout, enabled=ffn_lora, type="BertOutput"):
            model = cls(
                ...
                few_shot_prob_train=few_shot_prob_train,
                few_shot_prob_eval=few_shot_prob_eval,
